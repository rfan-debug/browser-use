...
task_cases = {
    ...
    4. Press the `right-arrowed` button to send it to the chat.
    5. Usually the chat window will returns a description of the video, click the "Ask anything" input box and immediately type in `proceed`, then press the `right-arrowed` button to send it to the chat.
    6. If a video content is generated as we expected, mark it as SUCCESS, otherwise, mark it as FAILED.
    ---
    ALWAYS REMEMBER the following: 
    1. Waiting on the loading page and never take actions there. 
   	2. NEVER touch anything on the canvas, focus on the chat message itself.
